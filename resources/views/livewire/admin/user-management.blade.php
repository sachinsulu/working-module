...
<div x-data="userManagement()" class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center gap-3 glass-card p-4 rounded-2xl flex-wrap">
        ...
        <select wire:model.live="filterDepartment" class="bg-slate-950 border border-slate-800/80 rounded-xl px-3 py-2.5 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition">
        ...
        <select wire:model.live="filterRole" class="bg-slate-950 border border-slate-800/80 rounded-xl px-3 py-2.5 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-800/40 text-sm">
                    @forelse($users as $user)
                        <tr wire:key="user-row-{{ $user->id }}" class="hover:bg-slate-900/20 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-[10px] text-slate-500 font-semibold uppercase tracking-wider mt-0.5">{{ $user->employee_id ?: 'NO EMP ID' }}</div>
                                    </div>
                                </div>
                            </td>

                            <td class="px-6 py-4">
                                <div class="font-medium text-slate-300">{{ $user->department ?: 'N/A' }}</div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-1">
                                    @forelse($user->roles as $role)
                                        <span class="px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-indigo-500/10 border border-indigo-500/20 text-indigo-300">{{ $role->name }}</span>
                                    @empty
                                        <span class="text-slate-500 text-xs italic">No roles</span>
                                    @endforelse
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button type="button" wire:click="toggleStatus({{ $user->id }})" class="p-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-emerald-400 hover:border-emerald-500/30 transition-all" title="Toggle Status">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                                    </button>
                                    <a href="{{ route('admin.users.edit', $user) }}" class="p-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-indigo-400 hover:border-indigo-500/30 transition-all" title="Edit details">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    @if($user->id !== auth()->id())
                                        <button type="button" @click="openDeleteModal({{ $user->toJson() }})" class="p-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-rose-400 hover:border-rose-500/30 transition-all" title="Delete account">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    ...
                                    <span class="text-xs">No matching system members found. Try refining search query.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm" style="display: none;">
        <div @click.away="showDeleteModal = false" class="glass-card bg-slate-900 border border-slate-800/80 w-full max-w-md rounded-3xl p-6 shadow-2xl">
            ...
            <h3 class="text-base font-bold text-white">Delete Member?</h3>
            <p class="text-xs text-slate-400 mt-2" x-text="`Are you sure you want to delete ${deletingUser.name}? This action cannot be undone.`"></p>
            ...
            <div class="mt-6 flex items-center justify-end gap-3">
                ...
                <form :action="'/admin/users/' + deletingUser.id" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
