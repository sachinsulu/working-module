        ...
        <div class="glass-card rounded-2xl p-6 relative overflow-hidden transition-all duration-300 hover:scale-[1.02] group">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-3xl font-extrabold text-white mt-2 group-hover:text-indigo-300 transition duration-300">{{ $stats['total_users'] }}</h3>
                </div>
        ...
        <div class="glass-card rounded-2xl p-6 relative overflow-hidden transition-all duration-300 hover:scale-[1.02] group">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-3xl font-extrabold text-white mt-2 group-hover:text-violet-300 transition duration-300">{{ $stats['total_roles'] }}</h3>
                </div>
        ...
        <div class="glass-card rounded-2xl p-6 relative overflow-hidden transition-all duration-300 hover:scale-[1.02] group">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-3xl font-extrabold text-white mt-2 group-hover:text-fuchsia-300 transition duration-300">{{ $stats['total_permissions'] }}</h3>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                @forelse($stats['departments'] as $dept)
                    <div class="space-y-2">
                        <div class="flex items-center justify-between text-xs font-semibold">
                            ...
                            <span class="text-indigo-400">{{ $dept['count'] }} member{{ $dept['count'] > 1 ? 's' : '' }}</span>
                        </div>
                        <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                            <div class="h-full bg-gradient-to-r from-indigo-500 to-fuchsia-500 rounded-full" style="width: {{ ($dept['count'] / max($stats['total_users'], 1)) * 100 }}%"></div>
                        </div>
                    </div>
                @empty
        ...
        <div class="lg:col-span-3 glass-card rounded-2xl p-6 space-y-6">
            ...
            <div class="divide-y divide-slate-800/60 space-y-4">
                @forelse($stats['recent_users'] as $rUser)
                    <div class="flex items-center justify-between pt-4 first:pt-0">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-bold text-xs uppercase border border-indigo-500/15">
                                {{ substr($rUser->name, 0, 2) }}
                            </div>
                            <div>
                                <h5 class="text-xs font-bold text-slate-200">{{ $rUser->name }}</h5>
                                <p class="text-[10px] text-slate-400 mt-0.5">{{ $rUser->email }} • {{ $rUser->department ?: 'Unassigned Dept' }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            @foreach($rUser->roles as $role)
