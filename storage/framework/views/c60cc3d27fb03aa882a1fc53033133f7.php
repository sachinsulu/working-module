...
<aside class="w-72 glass-sidebar flex flex-col h-full flex-shrink-0 relative overflow-hidden z-20">
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 relative z-10 overflow-y-auto">
        ...
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition duration-300 <?php echo e(request()->routeIs('admin.dashboard') ? 'bg-gradient-to-r from-indigo-500/15 to-indigo-600/5 border-l-2 border-indigo-500 text-indigo-300' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
        ...
        <a href="<?php echo e(route('admin.users.index')); ?>" class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition duration-300 <?php echo e(request()->routeIs('admin.users.index') ? 'bg-gradient-to-r from-indigo-500/15 to-indigo-600/5 border-l-2 border-indigo-500 text-indigo-300' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="<?php echo e(route('admin.clients.index')); ?>" class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition duration-300 <?php echo e(request()->routeIs('admin.clients.index') ? 'bg-gradient-to-r from-indigo-500/15 to-indigo-600/5 border-l-2 border-indigo-500 text-indigo-300' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="<?php echo e(route('admin.projects.index')); ?>" class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition duration-300 <?php echo e(request()->routeIs('admin.projects.index') ? 'bg-gradient-to-r from-indigo-500/15 to-indigo-600/5 border-l-2 border-indigo-500 text-indigo-300' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="<?php echo e(route('admin.roles.index')); ?>" class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition duration-300 <?php echo e(request()->routeIs('admin.roles.index') ? 'bg-gradient-to-r from-indigo-500/15 to-indigo-600/5 border-l-2 border-indigo-500 text-indigo-300' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
            </svg>
        ...
        <!-- Departments Link -->
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('view department stats')): ?>
        <a href="<?php echo e(route('admin.departments.index')); ?>" class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition duration-300 <?php echo e(request()->routeIs('admin.departments.index') ? 'bg-gradient-to-r from-indigo-500/15 to-indigo-600/5 border-l-2 border-indigo-500 text-indigo-300' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7h18M3 12h18M3 17h18" />
            </svg>
    ...
    <div class="p-4 border-t border-slate-800/60 relative z-10 bg-slate-950/40">
        <div class="flex items-center gap-3 p-2 rounded-xl bg-slate-900/60 border border-slate-800/40">
            ...
            <div class="overflow-hidden">
                <h4 class="text-xs font-bold text-slate-200 truncate"><?php echo e(Auth::user()->name); ?></h4>
                <p class="text-[10px] text-indigo-400 font-semibold truncate uppercase"><?php echo e(Auth::user()->getRoleNames()->first() ?? 'User'); ?></p>
            </div>
        </div>
    </div>
</aside>
<?php /**PATH D:\workmodule\longtail-WM\resources\views/layouts/partials/sidebar.blade.php ENDPATH**/ ?>