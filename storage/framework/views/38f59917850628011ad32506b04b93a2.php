            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-800/40 text-sm">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $projects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr <?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::$currentLoop['key'] = 'project-row-'.e($project->id).''; ?>wire:key="project-row-<?php echo e($project->id); ?>" class="hover:bg-slate-900/20 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-extrabold text-xs border border-indigo-500/15 shadow-inner shrink-0">
                                        <?php echo e(strtoupper(substr($project->project_name, 0, 2))); ?>

                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-200"><?php echo e($project->project_name); ?></div>
                                        <div class="text-[10px] text-slate-500 font-semibold uppercase tracking-wider mt-0.5">#<?php echo e($project->id); ?></div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-xs text-slate-400">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($project->start_date || $project->end_date): ?>
                                    <div><?php echo e($project->start_date?->format('M d, Y') ?? '—'); ?></div>
                                    <div class="text-slate-600">→ <?php echo e($project->end_date?->format('M d, Y') ?? 'Ongoing'); ?></div>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?php echo e(route('admin.projects.edit', $project)); ?>" class="p-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-indigo-400 hover:border-indigo-500/30 transition-all" title="Edit project">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <button type="button" @click="openDeleteModal(<?php echo e($project->toJson()); ?>)" class="p-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-rose-400 hover:border-rose-500/30 transition-all" title="Delete project">
...
<?php /**PATH D:\workmodule\longtail-WM\resources\views/livewire/admin/project-management.blade.php ENDPATH**/ ?>